function [EEG, options] = Trauma(input,opts)
    ...
    if (nargin < 1)
        ME = MException('Alakazam:Trauma','Problem in Trauma: No Data Supplied');
        throw(ME);
    end
    %% Called with options?
    if (nargin == 1)
    ...
    names = ["start HR Measure S1", "start QS11","end QS11","start Baseline HR",...
             "stop Baseline HR","start film",...
