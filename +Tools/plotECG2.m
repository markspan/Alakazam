...
if size(SIG.X0, 1) == 1, SIG.X0 = SIG.X0'; end
...
if nnz(~isfinite(SIG.Y0)) > 0.5 * numel(SIG.Y0)
    warning('Y contains %.2f %% infinite values\n', nnz(~isfinite(SIG.Y0)) / numel(SIG.Y0) * 100);
end
...
hs.line = plot(hs.ax, 1, 1:size(SIG.Y0, 2), lineSpec);
...
function redraw(varargin)
    ...
    startIndex = (N - numPoints) * scrollValue + 1;
    ...
    if size(YData, 1) / (axWidth_px * maxSamplesPerPixel) > 2
        ...
        YData = [min(YData, [], 1, 'includenan'); max(YData, [], 1, 'includenan')];
