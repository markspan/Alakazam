function [ varargout ] = plotEEG( varargin )
Tools.ShowSliders
% -------------------------------------------------------
%    Based on
%
...
if length(args)>=2 && isnumeric(args{1}) && isstruct(args{2})
  ...
  %  SIG.bl = SIG.Y0(1,:);
  %  SIG.Y0 = bsxfun(@minus,SIG.Y0,SIG.bl);
...
if numel(SIG.X0)==1 % X = sample rate
    validateattributes(SIG.X0,{'double'},{'nonempty','real','finite','scalar','positive'     }, 'plotEEG','scalar X',1)
else % X = timestamps
    validateattributes(SIG.X0,{'double'},{'nonempty','real','finite','vector','nondecreasing'}, 'plotEEG','vector X',1)
end
validateattributes(SIG.Y0,{'double'},{'nonempty','real','2d'}, 'plotEEG','Y')
...
%hs.line = plot(hs.ax,1,1:size(SIG.Y0,2),lineSpec, 'ButtonDownFcn',{@lineSelect, SIG.EEG}); % Safer: Don't allow unmatched name-value pairs. Plot can still be modified by handles.
...
hs.panel.Visible = 'on';
    ...
    function lineSelect(src, ~, EEG)
        ...
        % Plot only the selected channel: Use a new call to plotEEG (with the new parent).
        switch get(gcf, 'SelectionType')
            case 'open'
                ...
                if ~strcmp(get(gcf, 'Tag'),'SingleChannelAlakazamWindow');
                    ...
                    mczoom      = get(findobj(MultiChannelPlotFigure, 'Tag', 'Zoom')  , 'Value');
                    mcscale     = get(findobj(MultiChannelPlotFigure, 'Tag', 'Scale') , 'Value');
                    mcscroll    = get(findobj(MultiChannelPlotFigure, 'Tag', 'Scroll'), 'Value');
                    ...
                    CurrentPlot = plotEEG(data.times/1000, data, 'YLimMode', 'fixed','zoomval',mczoom,'scaleval',mcscale,'scrollval',mcscroll, 'ShowInformationList','none','ShowAxisTicks','on', 'Parent', SingleChannelPlotFigure);
                    %CurrentPlot = plotEEG(data.times/1000, data, 'ShowInformationList','none','ShowAxisTicks','on','YLimMode', 'dynamic', 'AutoStackSignals', {data.chanlocs.labels}, 'Parent', SingleChannelPlotFigure);
...
if ~strcmpi(parser.Results.ShowInformationList,'none')
    ...
    function delete_plotEEG(varargin)
...
hs.ax.DeleteFcn    = @delete_plotEEG;
hs.panel.DeleteFcn = @delete_plotEEG;
hs.zoom.DeleteFcn  = @delete_plotEEG;
