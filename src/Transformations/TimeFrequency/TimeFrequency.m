...
if ~isfield(input, 'DataFormat') || ~strcmpi(input.DataFormat, 'EPOCHED')
    throw(MException('Alakazam:TimeFrequency', sprintf([ ...
        'Problem in TimeFrequency: this needs single-trial epoched data (DataFormat = "EPOCHED"), ' ...
        'and not this dataset (DataFormat = "%s"). Would you run DefineBins with an ''epoch'' ' ...
...
if ~isfield(input, 'bindesc') || isempty(input.bindesc)
    throw(MException('Alakazam:TimeFrequency', ...
        'Problem in TimeFrequency: I''m afraid this dataset has no bins (EEG.bindesc is empty). Please run DefineBins first.'));
end
...
if opts.MaxFreq >= input.srate / 2
    throw(MException('Alakazam:TimeFrequency', sprintf([ ...
        'Problem in TimeFrequency: the maximum frequency (%.3g Hz) needs to sit below ' ...
        'the Nyquist frequency (%.3g Hz for this %g Hz dataset) -- would you lower it?'], ...
        opts.MaxFreq, input.srate / 2, input.srate)));
end
if opts.BaselineStop <= opts.BaselineStart
    throw(MException('Alakazam:TimeFrequency', ...
        'Problem in TimeFrequency: the baseline window''s stop time needs to come after its start time -- could you check those values?'));
end
