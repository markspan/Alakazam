...
if isempty(eegIdx)
    throw(MException('Alakazam:AutoGEDAI', ...
        ['I''m afraid none of this dataset''s channels match GEDAI''s standard 10-5 ' ...
         'electrode set, so there is nothing for GEDAI to denoise. ' ...
         'Please rename channels to match 10-5 nomenclature first.']));
end
if ~isempty(otherIdx)
...
%   the path already.
    ...
    if exist(elc, 'file') ~= 2
        throw(MException('Alakazam:AutoGEDAI', ...
            'I''m afraid GEDAI''s bundled electrode template could not be found at %s.', elc));
    end
end
