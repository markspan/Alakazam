...
%   TRANSTOOLS.DRAWBRAINMAP, TRANSTOOLS.TEMPLATESCALPLOCS.
    ...
    if ~isfield(EEG, 'DataFormat') || ~strcmpi(EEG.DataFormat, 'Averaged')
        throw(MException(errorId, sprintf([ ...
            'Problem in %s: this only works on an averaged ERP (a subject Average ' ...
            'or a Grand Average), not on this dataset (DataFormat = "%s"). Would you ' ...
            'run Average -- or Grand Average, for a group result -- on it ' ...
    ...
    if ~any(hasPos)
        throw(MException(errorId, sprintf([ ...
            'Problem in %s: I''m afraid none of this dataset''s channels match a ' ...
            'standard scalp position, so there is nothing to plot. Please rename ' ...
