...
%               lags*predictors, sparse), .X (modelled events x predictors),
%               .beta_dc (channels x lags x predictors), and .timelimits and
%               .srate, which place each lag on the recording exactly as
%               uf_timeexpandDesignmat placed it.
...
%   the mean of its corrected trials is exactly that bin's fitted waveform.
%   That holds up to the solver's convergence, for events in one bin only,
%   and with covariates only where the bin's own mean of the covariate is
%   the centre (otherwise the mean trial is the response at the bin's own
%   mean, which is what an average of those trials should be anyway), and
%   for the trials that are usable. It is the property the tests hold the
%   arithmetic to.
%
...
%   See also UNFOLD.FITBINS, DECONVOLVE.
    ...
    % The betas as one column per channel, in Xdc's own column order (lag
    % fastest, then predictor), and as one row per (channel, lag) for the
    % trials' own shares.
    flat = reshape(permute(model.beta_dc, [2 3 1]), nlags * npred, nchan);
