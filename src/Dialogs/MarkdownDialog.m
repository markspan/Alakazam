...
%   syntax; --no-highlight because the stylesheet below does the code
%   styling and pandoc's highlighter would inject a second, conflicting one.
    html = '';
    try
        [status, output] = system(sprintf( ...
            '"%s" --from=gfm --to=html --no-highlight "%s"', exe, mdFile));
    catch
