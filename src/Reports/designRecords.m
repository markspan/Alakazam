...
%   See also DERIVEDESIGN, REPORTDESIGNPLAN, ALAKAZAM.COLLECTDESIGNRECORDINGS.
    ...
    for k = 1:numel(entries)
        ...
        records(end + 1) = struct( ...
            'name', e.subject, ...
            'person', trimmedCharFieldOr(e, 'person', e.subject), ...
            'group', trimmedCharFieldOr(e, 'group', ''), ...
            'session', trimmedCharFieldOr(e, 'session', ''), ...
            'bins', {binLabels(e)}, ...
            'included', true, ...
            'file', ''); %#ok<AGROW>
    end
...
function value = trimmedCharFieldOr(s, name, default)
%TRIMMEDCHARFIELDOR  A CHAR accessor, not TransTools.FieldOr: it also rejects a value that is
%   blank once trimmed, and returns char. Named for what it does so that it
%   is not mistaken for the general helper.
