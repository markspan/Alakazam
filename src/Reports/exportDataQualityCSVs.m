...
%   file existing -- the same choice writeSmeCsv makes.
    ...
    fprintf(fid, ['dataset,group,session,step,item,n,n_total,pct,n_unique,' ...
        'channel_epochs,channels_tested,scope,threshold,components,' ...
        'n_samples_rejected,n_samples,sensai,enova_epoch_max,enova_epoch_median,' ...
        'enova_channel_max,n_excluded,pct_within_one,mean_offset_ms,detail\n']);
    for e = 1:numel(entries)
        ...
        for r = 1:numel(rows)
            ...
            fprintf(fid, '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n', ...
                ...
                csvField(row.step), csvField(row.item), ...
                num(row.n), num(row.n_total), num(row.pct), num(row.n_unique), ...
                num(row.channel_epochs), num(row.channels_tested), csvField(row.scope), ...
                num(row.threshold), csvField(row.components), ...
                num(row.n_samples_rejected), num(row.n_samples), num(row.sensai), ...
                num(row.enova_epoch_max), num(row.enova_epoch_median), ...
                num(row.enova_channel_max), num(row.n_excluded), ...
                num(row.pct_within_one), num(row.mean_offset_ms), ...
                csvField(row.detail));
        end
    end
end
...
function writeSummaryCsv(file, entries)
    ...
    fprintf(fid, ['dataset,group,session,bin,channel,n_trials,n_trials_rejected,' ...
        'pct_trials_rejected,n_flagged,pct_flagged,n_interpolated,pct_interpolated,' ...
        'baseline_sd_uv,sme_uv,' ...
        'subject_pct_trials_rejected,subject_pct_channel_epochs_flagged,' ...
        'subject_pct_channel_epochs_interpolated,' ...
        ...
        'subject_n_trials_rejected,subject_n_trials_truncated,subject_n_trials_kept\n']);
    for e = 1:numel(entries)
        entry = entries(e);
        s = entry.quality.subject;
        rows = entry.quality.byBinChannel;
        for r = 1:numel(rows)
            fprintf(fid, '%s,%s,%s,%s,%s,%d,%d,%s,%d,%s,%d,%s,%s,%s,%s,%s,%s,%s,%d,%d,%s,%s,%s,%d,%d,%d\n', ...
                csvField(entry.subject), csvField(entry.group), csvField(entry.session), ...
