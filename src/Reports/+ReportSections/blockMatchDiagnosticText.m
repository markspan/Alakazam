...
%
%   BLOCK_UNIT is set in the report's setup chunk: "time window" in an ERP
%   report, "frequency" in a spectral one, whose blocks are frequencies.
    s = strjoin({ ...
        'cat(sprintf("\n\n*(Matched %d of %d total subject(s) for this block (%s: %s), of which %d have at least one non-missing value. Distinct raw bin value(s) seen for this %s: %s.)*\n\n",' ...
        '            n_distinct(grp$dataset), n_distinct(dat$dataset), block_unit, "__WINDOW_R__",' ...
        '            n_distinct(grp$dataset[!is.na(grp$value)]), block_unit,' ...
