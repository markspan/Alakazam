...
%   this function only computes.
    ...
    if numel(sourceFiles) < 2
        throw(MException('Alakazam:GrandAverage', ...
            ['I''m afraid a grand average needs at least two subjects to combine. ' ...
             'Would you pick more datasets in the dialog?']));
    end
...
%   count and time count, and carry the same set of bin labels.
    ...
    for i = 2:numel(subjects)
        ...
        if ~isequal(subSize(1:3), refSize(1:3))
            throw(MException('Alakazam:GrandAverage', sprintf([ ...
                'Unfortunately, "%s" has a %s map of size %s (channels x freq x time), but "%s" ' ...
                'has %s. Every subject needs the same channels, frequencies and ' ...
                'time points -- would you run the transform with the same settings on each?'], ...
        ...
        if ~isequal(subjectLabels, referenceLabels)
            throw(MException('Alakazam:GrandAverage', sprintf([ ...
                '"%s" has bins %s, but "%s" has bins %s, I''m afraid. Every subject needs the ' ...
...
%   a silent size mismatch or a wrong bin lineup happen later.
    ...
    for i = 2:numel(subjects)
        ...
        if size(subject.data, 1) ~= size(reference.data, 1)
            throw(MException('Alakazam:GrandAverage', sprintf([ ...
                'I''m afraid "%s" has %d channels, but "%s" has %d. Every subject in a ' ...
        ...
        if size(subject.data, 2) ~= size(reference.data, 2)
            throw(MException('Alakazam:GrandAverage', sprintf([ ...
                'Unfortunately, "%s" has %d time samples per epoch, but "%s" has %d. Every ' ...
        ...
        if ~isequal(subjectLabels, referenceLabels)
            throw(MException('Alakazam:GrandAverage', sprintf([ ...
                '"%s" has bins %s, but "%s" has bins %s, I''m afraid. Every subject needs ' ...
                'the same set of bin labels -- would you make sure they all ran the ' ...
