...
%   into Data & Analyses's own per-subject branches.
    ...
    % The version suffix is the same string the About box shows (see
    % alakazamVersion), so the titlebar answers "which one is this" without
    % having to open About at all -- useful the moment two installs are open
    % side by side (see onUpdate's own note on path shadowing between them).
    this.MainFigure = uifigure( ...
        "Name",   sprintf("Alakazam %s", alakazamVersion().Version), ...
        "Tag",    "AlakazamApp", ...
