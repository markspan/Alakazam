...
%
%   Values persist across sessions via MATLAB preferences (group 'Alakazam').
%   The store is a lazily-created singleton, so reads and writes always see the
%   same in-memory values; SettingsDialog persists them on Save.
%
...
%   See also SETTINGSDIALOG.
    ...
    methods (Static)
        ...
        function save()
        %SAVE  Persist the current values to MATLAB preferences.
            obj = AlakazamSettings.instance();
            setpref(AlakazamSettings.Group, AlakazamSettings.PrefKey, obj.Values);
        end
    ...
    methods (Access = private)
        ...
        %REFRESH  Rebuild Values from the schema defaults overlaid with prefs.
            if ispref(AlakazamSettings.Group, AlakazamSettings.PrefKey)
                stored = getpref(AlakazamSettings.Group, AlakazamSettings.PrefKey);
            else
                stored = struct();
            end
    ...
    methods (Static, Access = private)
        ...
        %DEFINESCHEMA  The single source of truth for all settings.
            ...
            erpPlot = AlakazamSettings.section('erpPlot', 'ERP plot', [ ...
                ...
                AlakazamSettings.setting('confIntN', 'Confidence band (n x SE)', 'slider', 3, ...
                    ...
                    'limits', [1 5], 'enabledBy', 'showConfInt') ...
            ]);
            tabs(end + 1) = AlakazamSettings.tab('graphics', 'Graphics', erpPlot);
        end
        ...
        function item = setting(key, label, type, default, tooltip, varargin)
            ...
            % ([min max] for 'slider'/'number'), 'enabledBy' (key of a bool in
            % the same section that must be true for this control to be active).
            opts = struct('choices', {{}}, 'limits', [], 'enabledBy', '');
