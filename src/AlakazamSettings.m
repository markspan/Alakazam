    ...
    methods (Static, Access = private)
        ...
        %DEFINESCHEMA  The single source of truth for all settings.
            ...
            epochImage = AlakazamSettings.section('epochImage', 'Epoch image', [ ...
                ...
                AlakazamSettings.setting('reverseSort', 'Reverse the sort', 'bool', false, ...
                    ['If checked, an epoch image sorted with "Sort by" runs from the largest ' ...
                     'value at the top to the smallest at the bottom, the way EEGLAB''s ' ...
                     'erpimage draws it. If unchecked (default), the smallest is at the top. ' ...
                     'Trials without a value stay at the bottom either way.']) ...
            ]);
            colormapSection = AlakazamSettings.section('colormap', 'Colour map', [ ...
                AlakazamSettings.setting('name', 'Colour map', 'choice', 'diverging', ...
                    ...
                    'preview', @AlakazamSettings.colormapStrip) ...
            ]);
            ...
            % ---- Tab: Processing ----------------------------------------
            applyToAll = AlakazamSettings.section('applyToAll', 'Apply to All Raw Files', [ ...
                AlakazamSettings.setting('parallel', 'Process recordings in parallel', 'bool', true, ...
                    ['If checked, and the Parallel Computing Toolbox is installed, Apply to All ' ...
                     'Raw Files processes several recordings at once, each in its own MATLAB ' ...
                     'worker. The first run starts the workers, which takes a little while; ' ...
                     'they then stay ready for later runs. Unchecked processes one recording ' ...
                     'at a time in the app itself.']) ...
                AlakazamSettings.setting('maxWorkers', 'Most recordings at once (0 = automatic)', 'number', 0, ...
                    ['The largest number of recordings processed at the same time. Automatic (0) ' ...
                     'takes the number of cores and the free memory into account: each worker ' ...
                     'holds a whole recording and its intermediate results, so memory usually ' ...
                     'runs out before cores do.'], ...
                    'limits', [0 64], 'step', 1, 'enabledBy', 'parallel') ...
            ]);
            tabs(end + 1) = AlakazamSettings.tab('processing', 'Processing', applyToAll);
        end
        ...
        function item = setting(key, label, type, default, tooltip, varargin)
            ...
            % active), 'preview' (a function from a value to an RGB image,
            % which the Settings dialog draws beside the control and redraws
            ...
            item = struct('key', key, 'label', label, 'type', type, ...
                ...
                'step', opts.step, 'enabledBy', opts.enabledBy, 'preview', {opts.preview});
        end

        function img = colormapStrip(name)
            % The colour map NAME as a strip of colour, lowest values on the
            % left: the Settings dialog's preview of the "Colour map" choice.
            % Drawn by TransTools.DivergingColormap, the one function every
            % plot takes its map from, so the strip is what the plots show.
            cmap = TransTools.DivergingColormap(name);
            img = repmat(reshape(cmap, 1, size(cmap, 1), 3), 12, 1, 1);
        end
    end
end
