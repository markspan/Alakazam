    ...
    methods (Access = private)
        ...
        %   count, so every list here uses one.
            ...
            measurementsGroups = { ...
                ...
                struct('title', 'Source Regions', 'items', {this.sourceReportItems(iconsDir)}), ...
        ...
        function items = sourceReportItems(this, iconsDir)
        %SOURCEREPORTITEMS  "Source Region Report" on the Export/Report tab
        %   (see Alakazam.onExportSourceReport): the statistics over
        %   anatomical region time courses produced by Parcellate.
        %
        %   Its own group rather than a fourth Batch Export button, and its
        %   own document rather than a section of the ERP report, for the
        %   same reason: a "channel P7" row and a "left middle temporal
        %   gyrus" row are not the same kind of claim. One is measured, the
        %   other is modelled through a template head model, template
        %   electrodes and a template atlas -- and a report that put them in
        %   one table would be inviting the reader to forget which was which.
            items = {struct('id', 'sourceReport', 'label', 'Source Region Report', ...
                'tooltip', ['Export ERP measurements taken on parcellated (source-region) ' ...
                    'datasets and render a Quarto report on them'], ...
                'icon', this.encodeSvgFile(fullfile(iconsDir, 'SourceReport.svg')))};
        end
