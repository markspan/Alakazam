...
| `src/Views/TimeFrequencyView.m` | Grid of per-bin ERSP heatmaps (`TransTools.ComputeErsp`'s precomputed output), one channel at a time; up/down arrows, the mouse wheel or a "Channel:" dropdown pick it, an instant re-slice since every channel was computed up front. The app's first multi-tile-grid view. |
