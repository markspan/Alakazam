...
| `src/IO/` | Import / export / format conversion: erpset converters (`erpsetToAveraged`, `averagedToErpset`), the ERPLAB BDF importer (`erplabBdfToBinScript`), the reverse direction for a plain `.set` export (`onExportSet.m`'s own `alakazamBinsToEventList` converts `EEG.bindesc`/`.bini` to ERPLAB's own `EVENTLIST`, with `rewriteEpochedEventLatencies`/`ensureEventEpochField` making an epoched dataset's event latencies/`.epoch` field EEGLAB-correct first -- both needed regardless of whether there are any bins, since `eeg_checkset` otherwise prunes trial-anchor events as "out of bounds"), and the analysis-script export (`exportAnalysisScript` and its `matlabLiteral`/`nativeTransformCall`/`rawCode`/`transformDefaults` helpers, which write a standalone reproducible `.m` script rather than a Quarto/R document -- see `src/Reports/` for those). |
| `src/Reports/` | The Quarto/R reporting pipeline and the CSV exporters that feed it: `generateQuartoReport` (design-aware -- emits one tailored APA-styled section per window/bin group, with narrative prose, `gt` tables and embedded plots, instead of one generic branchy template), `generateClusterStatsReport`/`generateSourceClusterStatsReport`, `generateDataQualityReport`, `renderQuartoReport`, `reportToWord`, `reportDesignPlan`, `dataQualityMetrics`, `erpScoreSME`, `deriveDesign`/`designCellSpecs`/`designRecords`, the `export*CSV.m` writers (`exportMeasurementsCSV`, `exportSpectralCSV`, `exportSpectraCSV`, `exportTrialMeasurementsCSV`, `exportGrandAveragesCSV`, `exportClusterStatsCSVs`, `exportCoherenceCSVs`, `exportDataQualityCSVs`), and the `+ReportDoc` (shared APA/gt helpers, YAML header, package bootstrap) and `+ReportSections` (per-analysis-type Quarto section builders) packages. Split out of `src/IO/` once report generation outgrew what that folder's own name meant. |
| `src/Support/` | Small shared helpers (`eegChannelMask`, `channelGroup`, `guessChannelTypes`, `channelTypeFromLabel`, `multiSelectField`, `spectralFreqSpecs`, `MinMaxPyramid`, plus a batch of dialog/IO helpers consolidated in the August 2026 audit: `dialogChromeColors`, `intersectLabels`/`asCell`/`mergeSeedFields`, `linesFromText`/`textFromLines`, `csvField`/`numField`/`csvBinLabel`, `firstNonEmpty`, `measureRowTypes`/`measureRowValue`/`areaModeScope`, shared between `exportMeasurementsCSV` and `generateQuartoReport`). |
...
| `src/Electrodes/` | Vendored electrode-position templates: `standard_1005.elc` (FieldTrip's, byte-identical to the copy `TransTools.Template1005File` already resolves off the installed toolbox) and `standard_waveguard64_equidistant.elc` (ANT Neuro's 64-channel equidistant cap). Not read by `Template1005File` itself, which is unchanged and still prefers the installed toolbox copies -- Channel Editor's "Look up locations" is the one caller that reads this folder directly, via `AvailableElectrodeTemplates`. See `src/Electrodes/README.md` for provenance and licence. |
| `src/Transformations/` | Analysis plugins (`<Name>/` folders) and the `+TransTools` shared-helper package. Which of them wrap another toolkit and which implement their method here is recorded in [`Docs/transformation-provenance.md`](Docs/transformation-provenance.md), along with the command that verifies it. |
| `src/+uiextras/`, `src/Compat/`, `src/Icons/`, `src/webtree/`, `src/help/` | The dialog-widget helper package, compatibility shims, SVG icon sources, the WorkSpaceTree HTML build pipeline, and the in-app help page build pipeline. |
...
| `src/Views/EpochView.m` | ERP-image view of epoched multichannel data: a time x trial heatmap for the current channel (up/down arrows, the mouse wheel, or the "Channel:" dropdown jump straight to one), rows grouped by bin and, with "Sort by:", ordered by a per-trial value that `src/Views/epochSortKeys.m` finds (when the next or previous event of each type fell, recorded by `TransTools.EpochNeighbours` when `DefineBins` or `Deconvolve` cut the trials, since the epoched dataset keeps only each trial's own event; DefineBins' reaction time; or a numeric field of each trial's own event; a time is drawn across the image as a line), plus a trial-average trace below -- replaces an earlier overlaid-line-traces plot (which didn't scale past a handful of trials/channels) and its own since-removed "all channels, one trial" mode (replaces Tools.plotEpochedTimeMulti). |
| `src/Views/AverageView.m` | Trial-average view with error bands and overlay, a "Channel:" dropdown alongside the usual arrow-key/wheel stepping (replaces Tools.plotEpochedTimeMultiAverage). |
| `src/Views/FourierView.m` | Keyboard-driven frequency-domain view (one channel's spectrum at a time, band shading, zoom/pan; up/down arrows, the mouse wheel or the button row's "Channel:" dropdown step the channel, left/right the trial -- same interaction model AverageView uses; replaces Tools.plotFourier). |
| `src/Views/TimeFrequencyView.m` | Grid of per-bin ERSP heatmaps (`ComputeErsp`'s precomputed output), one channel at a time; up/down arrows, the mouse wheel or a "Channel:" dropdown pick it, an instant re-slice since every channel was computed up front. The app's first multi-tile-grid view. |
...
| `src/@WorkSpace/` | Per-format file loaders, `.wksp` session persistence, and construction of the app's **three** `WorkSpaceTree` instances (`Tree` for data & analyses, `GrandAveragesTree` for grand averages, `ReportsTree` for rendered reports -- see `CreateTreeComponent.m`). Selecting a node in one clears the other two's own selection (`Alakazam.deselectOtherTrees`), so only one tree ever shows a node highlighted at a time. |
...
| `src/Reports/exportGrandAveragesCSV.m` | Writes every Grand Average to one long-format, R-compatible CSV (`Alakazam.onExportGrandAverages`, Grand Average tab's "Export Grand Averages..." button) -- the app's first working export path (`Workspace.ExportsDirectory` used to be configured but never actually written to). |
| `src/@cursor/`, `src/@label/` | Small UI helper classes (plot cursors and labels). |
| `src/Transformations/<Name>/` | Analysis plugins. Each folder: `<Name>.m` (entry), `<Name>.json` (manifest), `<Name>.png` (icon), plus the helpers only that transformation uses, as plain functions beside it (`RESS/RESSFilter.m`, `TimeFrequency/ComputeErsp.m`, `Photodiode/detectDiodeOnsets.m`, ...). A helper moves into `+TransTools` when a second transformation, a view or a report starts calling it, and back out again when it stops -- so where a function lives says who uses it. Every folder is on the path (`Alakazam.setupDirectories` adds `Transformations` with `genpath`), so the helpers are called by their bare name and are testable on their own; a test reaches them by adding that one folder to its `PathFixture`. |
| `src/Transformations/+TransTools/` | Helpers **shared** between transformations, or used from a view or a report: `CreateFilter`, `WindowByName` (the taper-window dispatch `Fourier.m` calls), `progressbar`, `FillChanlocs`/`Dipfit1005File`/`TemplateScalpLocs` for scalp-position lookups, `DrawScalpMap` (a uiaxes-compatible port of EEGLAB's `topoplot()` used by `ScalpDistributionView`, needed because `topoplot()` itself only draws via gca/gcf-implicit state and was confirmed to silently draw nothing when targeted at a uiaxes hosted in a uitab), `DivergingColormap` (the shared blue/white/red colour scale `TimeFrequencyView` and `EpochView`'s ERP-image both use for a signed, zero-centred quantity), `AddSharedColorbar`/`ColorbarColumnWidth` (the dedicated-hidden-axes colorbar trick six views share, centred in a sized column), `BuildBinDropdown` (the "Bin:" label + dropdown row shared by `TimeScrubStrip` and `CoherenceTopographyView`), `BuildChannelDropdown` (its channel-picking counterpart, shared by every view that already steps a single channel with the arrow keys/wheel: `EpochView`, `AverageView`, `TimeFrequencyView`, `CoherenceView`, and `FourierView`/`SpectralMeasureView` via `Views/ZoomPanButtons`), `ResolveScalpDistribution`/`TickedScalpBins` (shared by `ScalpDistribution`/`Brain3D` and their views), `ReadBrainMeshNV`/`DrawBrainMap`/`DrawBrainPatch` (`Brain3D`'s scalp-projection mesh and its shared patch/lighting/view drawing), `BuildSourceForwardModel`/`ComputeSourceEstimate`/`DrawSourceMap`/`ensureFieldTrip` (`Brain3D`'s optional FieldTrip-based MNE source-estimate mode, see the README's own "3D brain view" section), and the August 2026 audit's `FieldOr`/`LabelsToIdx`/`InitGuard` (the options-struct-parsing boilerplate ~21 transformations now share). `ComboOrder` works out the order in which combination (difference) bins can be computed, and from which bins, for `Average`, `TimeFrequency`'s `ComputeErsp` and `Unfold.fitBins`, which each used to carry their own copy of that loop; `EpochNeighbours` records, when `DefineBins` or `Deconvolve` cuts trials, when the nearest event of every type fell before and after each one, which is what `EpochView` sorts by. `CheckOptions` was removed in that same audit, orphaned by the `IIRFilter`→`Filter` rename with zero remaining callers. |
| `src/Transformations/+Unfold/` | Everything about the Unfold toolbox (regression-based, overlap-corrected ERPs): `Unfold.isAvailable` (passive check), `Unfold.ensure` (consent-gated install of the pinned 1.3.1 archive), `Unfold.startToolbox` (runs the toolbox's own `init_unfold` quietly, handling the three things it does that a transformation must not: printing, warning about the submodules Alakazam deliberately omits, and opening EEGLAB's GUI when EEGLAB is absent) `Unfold.eyeEegMeasures` (what each event field EYE-EEG writes for an eye movement means: its unit, and whether it is linear or circular), `Unfold.eventCovariates` (what a recording's numeric event fields can offer a formula: its numeric event fields, described from the former where it knows them, and with latency/duration/bin tags deliberately left out), `Unfold.otherEventsChoice` (the one reader of the stored choice of event codes outside the bins, older templates' `modelOtherEvents` included, which Deconvolve, its dialog and `binModel` all ask), and the ones that do the work: `Unfold.binModel` (bins to model, each with its formula and every field a formula names checked against the bin's own events, toolbox-free, so the mapping can be tested and previewed on its own), `Unfold.designMatrix` (the one `uf_designmat` call, shared by the fit and the dialog's preview, which names the bin whose formula the toolbox refuses), `Unfold.fitBins` (the fit, packaged in Average's shape with every bin evaluated at the same values of its terms, as overlap-corrected trials laid out by DefineBins' own `cutEpochs`, or as the model's terms), `Unfold.predictionValues` (the values the terms are evaluated at, read the one way the fit and the dialog both use), `Unfold.overlapCorrectedTrials` (the trials' arithmetic, each event's recording minus every other event's fitted response, toolbox-free and on the toolbox's own sample grid) and `Unfold.requireContinuous` (the one precondition, in one place, because both `Deconvolve` and `binModel` check it and the message has to say which node to use instead). `Deconvolve/` is the transformation that uses them; it cannot live here for the mirror reason: the plugin contract dispatches `feval('<Name>', EEG, opts)`, which a package function does not satisfy, so it belongs in its own `<Name>/` folder. A **package** folder, not a plain one, for a structural reason `RibbonScanTest` pins: every non-package folder under `Transformations` is offered as a transformation, so helpers that are not one have to sit in a `+package`. |
| `src/Transformations/+EyeEeg/` | Everything about EYE-EEG, for the `EyeTracking` transformation: `EyeEeg.isAvailable` (passive; reattaches an install from an earlier session), `EyeEeg.ensure` (consent-gated install of the pinned v1.01 archive), `EyeEeg.attach` (the toolbox folder and its `internal/` folder, after EEGLAB), `EyeEeg.rawRecording` (which raw file a dataset was read from: `EEG.etc.alz.rawFile`, which the loaders now record through `src/Support/recordRawFile.m` and which travels down every branch, falling back to EEGLAB's own `.filepath`/`.filename`), `EyeEeg.findEyeFile` (the `.asc` with the recording's base name beside it, converting an `.edf` on a copy in a temporary folder only when `edf2asc` is installed, so the raw directory is never written to), `EyeEeg.parse` (`parseeyelink`, with an empty keyword meaning the INPUT lines), `EyeEeg.columns` (which eye-tracker columns a setting names, for the join and its dialog alike), `EyeEeg.triggerCodes` and `EyeEeg.sharedAnchors` (the EEG's trigger numbers read the way EYE-EEG reads them, and the first and last trigger both recordings share), and `EyeEeg.syncQuality` (EYE-EEG's table of per-trigger offsets turned into the numbers the transformation refuses on and the data-quality report prints, through `src/Reports/eyeTrackingRow.m`). A package folder for the same structural reason as `+Unfold`. |
| `src/Reports/rscripts/` | The R and Quarto the reports are made of, one template per block: `.R` is pure R (a library, or a chunk body, and valid R on its own), `.Rpart` is an R fragment that only closes inside its section, `.qmd` is a Quarto fragment (prose, fences and R together). A generator reads one with `ReportDoc.template` and splices it in with `ReportDoc.lines`, then fills its `__TOKEN__` placeholders as before; the text is **inlined at generation**, never sourced at render, so an exported report stays a single self-contained `.qmd`. About 960 lines of R used to live in MATLAB string literals, where every quote had to be doubled and nothing checked the syntax until a render. `ReportTemplatesTest` holds the names and the files to each other and hands every `.R` to R's own parser; see `src/Reports/rscripts/README.md`. |
...
| `src/help/` | Node.js build that renders `README.MD` into `src/AlakazamHelp.html`, the page the Home tab's **Help** button opens. The output is **not** committed (about 5 MB of embedded screenshots, regenerated whenever the README changes), so it must be built once per clone; `Alakazam.offerReadmeInstead` handles its absence. See `src/help/README.md`. |
| `src/Reports/dataQualityMetrics.m`, `src/Reports/erpScoreSME.m`, `src/Reports/exportDataQualityCSVs.m`, `src/Reports/generateDataQualityReport.m` | The per-subject data-quality report (Export/Report tab): rejection and truncation counts, per-trial noise, and standardized measurement error per measurement window, analytic for mean amplitude and bootstrapped for the non-linear scores. `erpScoreSME` carries its own scoring implementation and is pinned to `Measure.m`'s numbers by `tests/ErpScoreSMETest.m`. |
| `src/Support/busyGate.m` | Owns the single busy indicator `beginBusy` raises, so a transformation's own options dialog can suspend it while the dialog is on screen. Driven from `TransTools.InitGuard` and `TransformSettings.set`; without it, a modal progress dialog covers the settings in MATLAB Online. |
...
ToolGroup history this replaced)
  ...
  ├─ WorkSpace ──> loads raw files, builds three WorkSpaceTree instances:
  │     ├─ Tree             (data & analyses, EEG-struct nodes, DataTreePanel)
  │     ├─ GrandAveragesTree (grand averages, EEG-struct nodes, GrandAveragesTreePanel)
  │     └─ ReportsTree       (rendered Quarto reports, not EEG data, ReportsTreePanel)
  │     -- selecting a node in one clears the other two (Alakazam.deselectOtherTrees)
