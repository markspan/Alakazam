...
| `src/IO/` | Import / export / format conversion: erpset converters (`erpsetToAveraged`, `averagedToErpset`), the ERPLAB BDF importer (`erplabBdfToBinScript`), the CSV writers (`exportMeasurementsCSV`, `exportSpectralCSV`, `exportGrandAveragesCSV`) and the Quarto report generator (`generateQuartoReport`, design-aware: emits one tailored APA-styled section per window/bin group, with narrative prose, `gt` tables and embedded plots, instead of one generic branchy template). |
