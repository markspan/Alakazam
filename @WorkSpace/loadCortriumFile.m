...
setIcon(tn,this.RawFileIcon);
    ...
    function EEG = readRawCortrium(fname, id)
        ...
        channelNameList = {'EGC','Ecg 1','Ecg 2','Ecg 3','Acc X','Acc Y', 'Acc Z','Acc Magnitude',...
    ...
    function ibiindices = detectIbi(ecgsig, srate)
        %% https://nl.mathworks.com/help/wavelet/ug/r-wave-detection-in-the-ecg.html
 
        [b,a] = TransTools.CreateFilter('low', srate, 5, 12);
        ecgsig=filtfilt(b,a,double(ecgsig));
        [b,a] = TransTools.CreateFilter('high', srate, 30, 12);
        ecgsig=filtfilt(b,a,double(ecgsig));
        ...
        y = abs(y).^2;
        for i = 1:4
            y(y > (nanmean(y) + (4 * nanstd(y)))) = NaN;
        end
        [~,ibiindices] = findpeaks(Tools.nanzscore(y), 'MinPeakDistance',srate/2);
        %,'MinPeakHeight',1,...
        %        'MinPeakDistance',srate/8);
    ...
    function dataLoaded = loadAndFormatData()
        ...
        switch fileFormat
            ...
            case 'BLE 24bit'
                ...
                [C3.serialNumber, conf, serial_ADS, C3.eventCounter, C3.leadoff, C3.accel.dataRaw, C3.temp.dataRaw,  C3.resp.dataRaw, C3.ecg.dataRaw, ecg_serials] = Tools.c3_read_ble_24bit(full_path);
