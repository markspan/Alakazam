...
%   See also REPLAYBRANCH, REPLAYBRANCHONTARGETS, APPLYTOALLWORKERS.
    ...
    methods (Test)
        ...
        function theWorkerCountFollowsSettingsCoresAndMemory(testCase)
            ...
            testCase.verifyEqual(count('AvailableBytes', 13 * gb), 2, ...
                ['13 GB free: the 3.3 GB the recordings themselves suggest is below the 4 GB ' ...
                 'floor every worker is given, and 11 GB spare holds two of those.']);
            testCase.verifyEqual(count('AvailableBytes', 24 * gb, 'FileBytes', repmat(0.017 * gb, 1, 9)), 5, ...
                ['The floor is what tiny recordings need: 17 MB apiece would otherwise budget ' ...
                 '1.55 GB a worker and start eight, and a branch whose cost is the model it ' ...
                 'builds rather than the data it copies then takes the machine into swap.']);
