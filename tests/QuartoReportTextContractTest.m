...
%   Run with: runtests('tests/QuartoReportTextContractTest.m').
    ...
    methods (Test)
        ...
        %   (see lmmSection's own header comment). That choice lives
        ...
        %   without changing one word of its prose.
            ...
            testCase.verifySubstring(anovaTxt, ...
                'lmerTest::lmer(value ~ bin + (1 + bin | person_id), data = d,', ...
                'The maximal (random intercept AND slope) LMM formula is missing.');
            testCase.verifySubstring(anovaTxt, ...
                'lmerTest::lmer(value ~ bin + (1 | person_id), data = d,', ...
        ...
        %   runtime.
            allowed = {'between_group_combo', 'indep_t', 'lmm_mixed', 'lmm_session', ...
                'lmm_within', 'one_sample_vs_0', 'paired_t', 'welch_anova'};
        ...
        %   it did not actually apply.
            ...
            % The summary's prose was rewritten to explain what a family is
            % and why only primary tests are in it. What this contract needs
            % from it is unchanged: that it names the correction the code
            % applies, and that it applies it to the primary family alone.
            testCase.verifySubstring(txt, 'corrected with the Benjamini-Hochberg procedure', ...
                'The summary prose no longer names the Benjamini-Hochberg correction it applies.');
            testCase.verifySubstring(txt, 'The **primary family** is the set of all primary tests', ...
                'The summary prose no longer says the correction applies to the primary family.');
            testCase.verifySubstring(txt, 'left out of the family', ...
                'The summary prose no longer says secondary tests are excluded from the correction.');
        end
